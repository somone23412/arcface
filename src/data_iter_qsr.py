...
class MultiResolutionFaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
        ...
        super(MultiResolutionFaceImageIter, self).__init__()
        assert path_imgrec
        if path_imgrec:
    ...
    def next(self):
        ...
        lr_batch_data = nd.empty((batch_size, c, h, w))
        ...
        try:
            while i < batch_size:
                ...
                for datum in data:
                    ...
                    batch_label[i][:] = label
                    lr_batch_data[i][:] = self.postprocess_data(self.generate_LR(datum))
                    lr_batch_data[i][:] = label
                    i += 1
        except StopIteration:
            if i<batch_size:
                raise StopIteration

        return batch_data, lr_batch_data, batch_label, batch_size - i
    ...
    def generate_LR(self, datum):
        """Final postprocessing step before image is loaded into the batch."""
        _rd = random.randint(0, 2)
        datum=datum.asnumpy()
        if _rd==0:
            datum=cv2.resize(datum,(16,16),interpolation=cv2.INTER_CUBIC)
            datum = cv2.resize(datum, (112, 112), interpolation=cv2.INTER_CUBIC)
        elif _rd==1:
            datum = cv2.resize(datum, (32, 32), interpolation=cv2.INTER_CUBIC)
            datum = cv2.resize(datum, (112, 112), interpolation=cv2.INTER_CUBIC)
        else:
            datum = cv2.resize(datum, (64, 64), interpolation=cv2.INTER_CUBIC)
            datum = cv2.resize(datum, (112, 112), interpolation=cv2.INTER_CUBIC)
        return mx.nd.array(datum)